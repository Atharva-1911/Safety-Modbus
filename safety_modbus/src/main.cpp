...
// CONVENIENCE CONSTANTS: RELAY MASKS
...
// Startup mask: all drivers and brakes engaged
...
// Spinlocks for thread-safe access to state variables
// (shared between debounce tasks and main loop)
...
bool    estop_stable_state  = false;   // true = E-Stop pressed, false = released
                                       // (Pin: LOW = pressed, HIGH = released)
...
void limit_debounce_task(void *pvParameters) {
    ...
    while (true) {
        ...
        taskENTER_CRITICAL(&limit_state_spinlock);   // Acquire spinlock
        ...
        taskEXIT_CRITICAL(&limit_state_spinlock);    // Release spinlock
...
/**
 * Debounce task for the E-Stop button.
 *
 * This task sleeps until woken by the ISR. When woken:
 *   1. Waits DEBOUNCE_TIME_MS for signal to stabilize.
 *   2. Reads the GPIO state (now stable after debounce).
...
void estop_debounce_task(void *pvParameters) {
    while (true) {
        ...
        // READ STABLE STATE: After debounce period, read actual GPIO level.
        // E-Stop pin: LOW = pressed (active), HIGH = released (INPUT_PULLUP)
        bool estop_pressed = (digitalRead(ESTOP_PIN) == LOW);

        // UPDATE STABLE STATE: Must be thread-safe since main loop also reads.
        taskENTER_CRITICAL(&estop_state_spinlock);   // Acquire spinlock
        ...
        taskEXIT_CRITICAL(&estop_state_spinlock);    // Release spinlock
...
void IRAM_ATTR estop_isr() {
    ...
    // If woken task has higher priority than current task, switch immediately
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
}

// Generic limit switch handler
void IRAM_ATTR handle_limit_isr(uint8_t index) {
 ...
 *
 * Debounce task has already filtered state with software 50 ms debounce.
...
void setup() {
    ...
    // ── Modbus RTU over HardwareSerial2 
    ...
    if (digitalRead(ESTOP_PIN) == LOW) {
        ...
        estop_stable_state      = false;  // released
    ...
    xTaskCreatePinnedToCore(
        ...
        3,                            // Priority
        &estop_debounce_task_handle,  // Task handle
        0                             // Core 0 (dedicated safety core)
    );
    Serial.println("Created debounce task for E-Stop");

    // Limit switch debounce tasks (one per switch, all on Core 0, priority 2)
    for (uint8_t i = 0; i < NUM_LIMIT_SWITCHES; i++) {
        ...
        *pIndex = i;
        ...
        xTaskCreatePinnedToCore(
            ...
            task_name,
            2048,                              // Stack size in bytes
            (void *)pIndex,                    // Index parameter
            2,                                 // Priority
            &limit_debounce_task_handles[i],   // Task handle
            0                                  // Core 0
        );
    ...
    // ── Initial relay state 
    if (estop_stable_state) {
        relay_state = 0xFFFF;        // E-STOP pressed → all relays OFF
    } else {
...
void loop() {
    ...
    if (estop) {
        ...
        // If coming out of E-Stop and modbus_value is all OFF,
        // restore startup mask automatically
        if (modbus_value == 0xFFFF) {
    ...
    // Gripper priority: always follow Modbus command for gripper bit
    ...
    // Maintain ~1 kHz loop rate for fast response
    delay(1);
}